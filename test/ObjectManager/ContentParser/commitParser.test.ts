...
describe('Test commitParser function', () => {
    test('for parsing the loose commit object properly.', () => {
        ...
        const startIdx = 11;
        const endIdx = 279;
        const hash = '03ad3ec20a59305503cdcef6667e50fa94edc563';

        const content = readFileSync(filePath);
        const decryptedBuf = inflateSync(content);
        const body = decryptedBuf.subarray(startIdx, endIdx);
        const data = commitParser(body, hash);

        expect(data.hashes.treeHash).toBe('56d45faf43f5e5acd80836af1d00edddeb0bc8a2');
    });
    ...
    test('for parsing the packed commit object properly.', () => {
        ...
        const data = commitParser(decryptedBuf, hash);
        
        expect(data.hashes.treeHash).toBe('d1b421f8873e2dcedabdec670428910ba1bc51d3');
    });
});
